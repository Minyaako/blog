...
朋友圈使用紧凑条目，默认展示最近 6 篇，可展开查看快照中的其余文章，再次点击可收起。

交换友链信息下方的「友链留言板」复用 Waline 评论，使用独立且固定的 `/friends/` 讨论串，不与文章评论混用。访客可填写站点名称、网址、简介、头像地址和 RSS；站长确认后手动加入配置，留言不会自动变为友链。评论接近视口时才加载，支持页面切换后重新挂载；公开预览环境停用评论。
