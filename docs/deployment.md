...
7. 只有匿名拉取、主机边界、生产 Secrets 和临时网关都验证通过后，才打开闸门并手动调度当前 `main`：
   ...
   镜像构建与推送步骤设有 12 分钟超时，避免镜像仓库上传停滞时无限占用发布任务。超时后先检查日志以及仓库中该 SHA 的镜像清单：清单已存在时可按上述重跑流程验证并部署；清单不存在时使用新提交发布。2026-09-15 的运行 `34864592179` 在构建完成后停于 `pushing layers` 超过 20 分钟，取消后确认未产生清单，生产仍保留上一健康版本。
