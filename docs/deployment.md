...
7. 只有匿名拉取、主机边界、生产 Secrets 和临时网关都验证通过后，才打开闸门并手动调度当前 `main`：
   ...
   镜像构建设有 12 分钟超时；上传步骤为 25 分钟，内部 Skopeo 进程最多 24 分钟，留出校验和清理时间。首次发布用 BuildKit 构建一次带 provenance 的 OCI 归档，再由 Skopeo 全量复制到 CCR；`scripts/publish-oci.mjs` 检查构建证明并比对归档 index、上传结果与远端清单的摘要。认证仅复用现有 Docker 登录文件，不读取或输出凭据；原 SHA 已存在时只接受相同摘要，拒绝覆盖不同镜像。鉴权或网络错误不会被当成“镜像不存在”。

   2026-09-15 的上传曾用约 9 分钟成功；运行 `34921513155` 在分层传输后触及原 11 分钟进程时限，且远端没有目标清单。扩大上传窗口是对慢速传输的缓解，并不证明网络吞吐问题已解决；仍需观察每次实际发布结果。不会因为超时而覆盖已有 SHA 或绕过检查部署。

   超时后先检查日志以及仓库中该 SHA 的镜像清单：清单已存在时可按上述重跑流程验证并部署；清单不存在时使用新提交发布。Skopeo 要求至少 1.9.0，保留 `--all --preserve-digests`，不得增加格式转换或移除构建证明。替代上传路径的本地验收与故障背景见 [镜像发布验证](verification/2026-09-15-image-publishing.md)。
