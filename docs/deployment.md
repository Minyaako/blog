...
7. 只有匿名拉取、主机边界、生产 Secrets 和临时网关都验证通过后，才打开闸门并手动调度当前 `main`：
   ...
   镜像构建与上传分别设有 12 分钟超时。首次发布用 BuildKit 构建一次带 provenance 的 OCI 归档，再由 Skopeo 全量复制到 CCR；`scripts/publish-oci.mjs` 检查构建证明并比对归档 index、上传结果与远端清单的摘要。认证仅复用现有 Docker 登录文件，不读取或输出凭据；原 SHA 已存在时只接受相同摘要，拒绝覆盖不同镜像。鉴权或网络错误不会被当成“镜像不存在”。
