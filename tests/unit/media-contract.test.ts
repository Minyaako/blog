...
describe('media publishing contract', () => {
  it('pins the shared publisher and records seven legacy objects plus hosted media', () => {
    ...
    expect(manifest.assets.filter((asset: { mode?: string }) => asset.mode !== 'hosted')).toHaveLength(7)
    expect(manifest.assets.filter((asset: { mode?: string }) => asset.mode === 'hosted')).toHaveLength(1)
