...
describe('production Node runtime contract', () => {
  it('uses a pinned non-root standalone runtime and checks the schema before startup', () => {
    ...
    expect(dockerfile).toContain('RUN node scripts/prepare-runtime.mjs')
    expect(dockerfile).toContain('COPY --from=build --chown=node:node /app/.runtime-node_modules ./node_modules')
    expect(dockerfile).not.toContain('/app/node_modules ./node_modules')
    expect(dockerfile).toContain('source=/app/scripts/runtime-smoke.mjs,target=/tmp/runtime-smoke.mjs')
    expect(dockerfile).toContain('node /tmp/runtime-smoke.mjs')
