...
async function confirmOnPage(page: Page, title: string, action: string) {
...
test('site dialogs cancel safely, restore focus and fit both themes without native prompts', async ({ page }, testInfo) => {
  ...
  await page.getByLabel('榜单标题', { exact: true }).fill('取消后必须保留的草稿')
  await names(page).first().fill('保留这个条目')
  await expect(page.locator('[data-save-status]')).toHaveText('已保存到此浏览器')
  const trigger = page.getByRole('button', { name: '丢弃草稿', exact: true })
  await trigger.click()
  const modal = page.getByRole('dialog', { name: '丢弃本地草稿', exact: true })
  ...
  for (const theme of ['light', 'dark']) {
    ...
    await page.screenshot({ path: testInfo.outputPath(`site-dialog-${theme}.png`) })
  }
  await page.keyboard.press('Escape')
  await expect(modal).toHaveCount(0)
  await expect(trigger).toBeFocused()
  await expect(page.getByLabel('榜单标题', { exact: true })).toHaveValue('取消后必须保留的草稿')
  await page.reload()
  const choice = page.getByRole('dialog', { name: '发现本地草稿', exact: true })
  await expect(choice).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(page.locator('[data-site-dialog]')).toHaveCount(0)
  await page.getByRole('button', { name: '创建我的榜单', exact: true }).click()
  await confirmOnPage(page, '发现本地草稿', '继续草稿')
  await expect(page.getByLabel('榜单标题', { exact: true })).toHaveValue('取消后必须保留的草稿')
  expect(nativeDialogs).toEqual([])
})

test('unsaved navigation uses a cancellable site dialog instead of browser unload UI', async ({ page }) => {
  const nativeDialogs: string[] = []
  page.on('dialog', dialog => { nativeDialogs.push(dialog.type()); void dialog.dismiss() })
  await page.addInitScript(() => Object.defineProperty(window, 'indexedDB', { get() { throw new Error('storage disabled') } }))
  await page.goto('/ranking/new/'); await editorReady(page)
  await page.getByLabel('榜单标题', { exact: true }).fill('未保存仍可取消离开')
  await page.locator('.ranking-nav a[href="/ranking/"]').click()
  const modal = page.getByRole('dialog', { name: '离开编辑页', exact: true })
  await expect(modal).toBeVisible()
  await modal.getByRole('button', { name: '留在此页' }).click()
  await expect(page).toHaveURL(/\/ranking\/new\/$/)
  await expect(page.getByLabel('榜单标题', { exact: true })).toHaveValue('未保存仍可取消离开')
  // No native browser warning is registered, even with unavailable draft storage.
  ...
  await page.locator('.ranking-nav a[href="/ranking/"]').click()
  await confirmOnPage(page, '离开编辑页', '仍要离开')
...
test('local recovery uses revision conflicts instead of overwriting another tab', async ({ page, context }) => {
  ...
  await other.goto('/ranking/new/'); await confirmOnPage(other, '发现本地草稿', '继续草稿'); await editorReady(other)
...
test('discard and recreate cannot let an old tab overwrite the replacement draft', async ({ page, context }) => {
  ...
  await confirmOnPage(other, '丢弃本地草稿', '丢弃草稿')
...
test('session discovery failure still permits a recoverable anonymous export', async ({ page, context }) => {
  ...
  await confirmOnPage(other, '恢复草稿文件', '恢复并替换')
...
test('unknown response retries original snapshot, then reject/edit/approve and visibility round trip', async ({ browser }, testInfo) => {
  ...
  await confirmOnPage(page, '发现本地草稿', '继续草稿'); await editorReady(page)
  await expect(page.getByLabel('榜单标题', { exact: true })).toHaveValue(title + ' 后续修改')
  ...
  await confirmOnPage(review, '发布榜单', '确认发布')
  ...
  await review.getByRole('dialog', { name: '下架榜单', exact: true }).getByLabel('下架原因').fill('验收隐藏及恢复')
  await confirmOnPage(review, '下架榜单', '确认下架')
  ...
  await review.getByRole('dialog', { name: '恢复榜单', exact: true }).getByLabel('恢复原因').fill('验收隐藏及恢复')
  await confirmOnPage(review, '恢复榜单', '确认恢复')
